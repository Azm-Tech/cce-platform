/**
 * Node icon registry — maps an InteractiveMapNode `iconKey` to a
 * lucide-style line-icon, rendered as an SVG data URI for Cytoscape
 * background-image. The Figma "knowledge map" nodes each carry a white
 * line icon inside the circle; the API gives us a string key, so we
 * resolve it here to inline SVG path markup.
 *
 * Cytoscape renders to <canvas> and cannot resolve CSS custom properties
 * or `currentColor`, so the stroke colour is read off :root at call time
 * (same pattern as cytoscape-styles.ts `token()`). The hex fallback is a
 * last resort for non-browser test contexts only.
 */

/** Resolve the icon stroke colour from the design tokens (light cyan). */
 ...
 * Keys are normalised lowercase; many API keys alias onto one icon.
 */
...
/** Normalise an iconKey: lowercase, trim, collapse spaces/underscores to dashes. */
...
/**
 * Returns an SVG data URI for the given iconKey, suitable for use as a
 * Cytoscape `background-image`. Unknown keys resolve to a neutral dot icon.
 */
export function iconDataUri(iconKey: string | null | undefined): string {
  const paths = ICON_PATHS[normalizeKey(iconKey)];
  const stroke = iconStroke();
  // Cytoscape rasterizes a background-image at the SVG's intrinsic size, then
  // scales that bitmap onto the node. An SVG with only a viewBox rasterizes
  // tiny (~24px) and upscales blurry — so declare an explicit high-resolution
  // width/height (the viewBox keeps the path coordinates intact) to stay crisp.
  const SIZE = 96;
  const svg =
