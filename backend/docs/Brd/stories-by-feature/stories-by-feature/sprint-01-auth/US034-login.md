...
- `login.page.ts` rewritten with ReactiveFormsModule + `POST /api/auth/login`
- JWT tokens stored: access token in memory signal, refresh token in `localStorage` (`cce_rt`)
- `AuthService.setSession()` + `loadCurrentUser()` called on success; navigates to `returnUrl`
- ERR020/ERR021 error keys mapped; demo-persona buttons removed
