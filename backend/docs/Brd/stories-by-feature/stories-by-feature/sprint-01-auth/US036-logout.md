...
- `AuthService.signOut()` calls `POST /api/auth/logout` with stored refresh token
- Clears access token signal + `cce_rt` from localStorage; navigates to `/`
- CON015 toast on success, ERR024 toast on API failure
- Header "Sign out" button already delegates to `AuthService.signOut()`
