...
/// <summary>
/// Typed <see cref="IResult"/> that wraps a <see cref="Result{T}"/> created envelope
...
public readonly struct CreatedApiResult<T> : IResult, IEndpointMetadataProvider
{
    ...
    public CreatedApiResult(Result<T> payload) => _payload = payload;

    public Task ExecuteAsync(HttpContext httpContext)
    {
        if (_payload.IsSuccess)
        {
            return Results.Json(_payload, statusCode: StatusCodes.Status201Created).ExecuteAsync(httpContext);
        }
    ...
    {
        builder.Metadata.Add(new ProducesResponseTypeMetadata(StatusCodes.Status201Created, typeof(Result<T>), ["application/json"]));
