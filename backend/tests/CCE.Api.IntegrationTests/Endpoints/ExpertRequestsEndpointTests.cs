...
public class ExpertRequestsEndpointTests :
    ...
    IClassFixture<AdminAuthFixture>
{
    ...
    public ExpertRequestsEndpointTests(
    ...
    public async Task Anonymous_request_returns_401()
    {
        ...
        var resp = await client.GetAsync(new Uri("/api/admin/expert-requests", UriKind.Relative));
    ...
    public async Task SuperAdmin_request_returns_200_with_paged_shape()
    {
        ...
        var resp = await client.GetAsync(new Uri("/api/admin/expert-requests?page=1&pageSize=20", UriKind.Relative));
    ...
    {
        using var client = _factory.CreateClient();
        using var body = System.Net.Http.Json.JsonContent.Create(new { academicTitleAr = "أستاذ", academicTitleEn = "Professor" });

        var resp = await client.PostAsync(
            new Uri($"/api/admin/expert-requests/{System.Guid.NewGuid()}/approve", UriKind.Relative), body);

        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact]
    public async Task Approve_with_unknown_id_returns_404()
    {
        using var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _auth.AccessToken);
        using var body = System.Net.Http.Json.JsonContent.Create(new { academicTitleAr = "أستاذ", academicTitleEn = "Professor" });
