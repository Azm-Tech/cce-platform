...
public class UploadAssetCommandHandlerTests
{
    ...
    {
        var sut = BuildSut(out _, out var scanner, out var db, currentUserId: System.Guid.NewGuid());
        scanner.ScanAsync(default!, default).ReturnsForAnyArgs(VirusScanResult.Clean);

        var result = await sut.Handle(BuildCommand("x.pdf", "application/pdf"), CancellationToken.None);
    ...
    {
        var sut = BuildSut(out var storage, out var scanner, out var db, currentUserId: System.Guid.NewGuid());
        ...
        result.Success.Should().BeTrue();
        result.Data!.VirusScanStatus.Should().Be(VirusScanStatus.Infected);
    ...
    public async Task Marks_scan_failed_when_scanner_returns_scan_failed()
    {
        ...
        result.Success.Should().BeTrue();
        result.Data!.VirusScanStatus.Should().Be(VirusScanStatus.ScanFailed);
    ...
    public async Task Buffers_content_and_passes_size_through()
    {
        ...
        var result = await sut.Handle(new UploadAssetCommand(content, "x.txt", "text/plain", payload.Length), CancellationToken.None);

        result.Success.Should().BeTrue();
        result.Data!.SizeBytes.Should().Be(payload.Length);
        result.Data.OriginalFileName.Should().Be("x.txt");
        ...
        System.Guid? currentUserId)
    {
        ...
        var service = Substitute.For<IAssetRepository>();
