...
public class ListHomepageSectionsQueryHandlerTests
{
    ...
    {
        var db = BuildDb(Array.Empty<HomepageSection>());
    ...
    {
        var hero = HomepageSection.Create(HomepageSectionType.Hero, 0, "ar-hero", "en-hero");
        var news = HomepageSection.Create(HomepageSectionType.FeaturedNews, 1, "ar-news", "en-news");

        var db = BuildDb([hero, news]);
        var sut = new ListHomepageSectionsQueryHandler(db);
    ...
    [Fact]
    public async Task Returns_both_active_and_inactive_sections()
    {
        var active = HomepageSection.Create(HomepageSectionType.Hero, 0, "ar-hero", "en-hero");
